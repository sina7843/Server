...
export class PublicGuidesController {
  ...
  @Get()
  async listGuides(@Query() query: unknown): Promise<PublicPostListResponse> {
    const q = parsePublicListQuery(query);
    const { items, total, page, limit } = await this.service.listPublished('guide', q);
    return toPublicPostListResponse(items, total, page, limit);
  }

  @Get(':slug')
  async getGuide(@Param('slug') slug: string): Promise<PublicPostResponse> {
    const post = await this.service.getPublished('guide', slug);
