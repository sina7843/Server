...
export function getAuthConfig(env: AuthConfigEnv = process.env): AuthConfig {
  return {
    ...
    refreshTokenTtlDays: readPositiveIntegerEnv(
      env,
      'AUTH_REFRESH_TOKEN_TTL_DAYS',
      { min: 1, max: 90 },
    ),
    passwordMinLength: readPositiveIntegerEnv(
      env,
    ...
    otpResendCooldownSeconds: readPositiveIntegerEnv(
      env,
      'AUTH_OTP_RESEND_COOLDOWN_SECONDS',
      { min: 60, max: 120 },
    ),
    otpDailyPhoneLimit: readPositiveIntegerEnv(
      env,
      'AUTH_OTP_DAILY_PHONE_LIMIT',
      { min: 1, max: 20 },
    ),
    ...
    jwtSecret: readStringWithMinLengthEnv(
      env,
      'AUTH_JWT_SECRET',
