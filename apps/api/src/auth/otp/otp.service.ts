...
export class OtpChallengeService {
  isExpired(challenge: Pick<OtpChallengeState, 'expiresAt'>, now = new Date()): boolean {
  ...
  hasAttemptsRemaining(challenge: Pick<OtpChallengeState, 'attempts' | 'maxAttempts'>): boolean {
    return challenge.attempts < challenge.maxAttempts;
  }

  canAttemptVerification(challenge: OtpChallengeState, now = new Date()): boolean {
    return (
  ...
  canResend(challenge: Pick<OtpChallengeState, 'nextResendAt'>, now = new Date()): boolean {
    return !challenge.nextResendAt || challenge.nextResendAt.getTime() <= now.getTime();
  }

  assertCanAttemptVerification(challenge: OtpChallengeState, now = new Date()): void {
    if (!this.canAttemptVerification(challenge, now)) {
      throw new Error('OTP challenge cannot be used for verification.');
    }
  }

  assertCanResend(challenge: Pick<OtpChallengeState, 'nextResendAt'>, now = new Date()): void {
