...
describe('auth security regression', () => {
  ...
  const authService = {
    ...
    verifyPhone: jest
      .fn()
      .mockRejectedValue(new BadRequestException('Phone verification could not be completed.')),
    ...
    forgotPassword: jest
      .fn()
      .mockResolvedValue(
        createGenericAuthResponse('If the phone is eligible, a password reset code will be sent.'),
      ),
    verifyResetOtp: jest
      .fn()
      .mockRejectedValue(new BadRequestException('Password reset could not be completed.')),
    resetPassword: jest
      .fn()
      .mockResolvedValue(createGenericAuthResponse('Password reset completed.')),
    logout: jest.fn(),
    logoutAll: jest.fn(),
  ...
  it('keeps registration generic and rejects unknown fields', async () => {
    ...
    const unknownFieldResponse = await fetch(`${baseUrl}/api/v1/auth/register`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        phone: '+98 912 000 0000',
